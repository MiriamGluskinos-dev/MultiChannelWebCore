...
*/
function printIteratorEntries$1(iterator, config, indentation, depth, refs, printer, separator = ": ") {
	let result = "";
	let width = 0;
...
*/
function printIteratorValues$1(iterator, config, indentation, depth, refs, printer) {
	let result = "";
	let width = 0;
	let current = iterator.next();
	if (!current.done) {
...
*/
function printListItems$1(list, config, indentation, depth, refs, printer) {
	let result = "";
...
* without surrounding punctuation (for example, braces)
*/
function printObjectProperties$1(val, config, indentation, depth, refs, printer) {
	let result = "";
	const keys = getKeysOfEnumerableProperties$1(val, config.compareKeys);
	if (keys.length > 0) {
		result += config.spacingOuter;
		const indentationNext = indentation + config.indent;
...
const asymmetricMatcher$1 = typeof Symbol === "function" && Symbol.for ? Symbol.for("jest.asymmetricMatcher") : 1267621;
const SPACE$2$1 = " ";
const serialize$5$1 = (val, config, indentation, depth, refs, printer) => {
	const stringedValue = val.toString();
	if (stringedValue === "ArrayContaining" || stringedValue === "ArrayNotContaining") {
		if (++depth > config.maxDepth) {
			return `[${stringedValue}]`;
		}
		return `${stringedValue + SPACE$2$1}[${printListItems$1(val.sample, config, indentation, depth, refs, printer)}]`;
	}
	if (stringedValue === "ObjectContaining" || stringedValue === "ObjectNotContaining") {
		if (++depth > config.maxDepth) {
			return `[${stringedValue}]`;
		}
		return `${stringedValue + SPACE$2$1}{${printObjectProperties$1(val.sample, config, indentation, depth, refs, printer)}}`;
	}
	if (stringedValue === "StringMatching" || stringedValue === "StringNotMatching") {
		return stringedValue + SPACE$2$1 + printer(val.sample, config, indentation, depth, refs);
	}
...
const test$5$1 = (val) => val && val.$$typeof === asymmetricMatcher$1;
const plugin$5$1 = {
	serialize: serialize$5$1,
	test: test$5$1
};

const SPACE$1$1 = " ";
const OBJECT_NAMES$1 = new Set(["DOMStringMap", "NamedNodeMap"]);
const ARRAY_REGEXP$1 = /^(?:HTML\w*Collection|NodeList)$/;
function testName$1(name) {
	return OBJECT_NAMES$1.has(name) || ARRAY_REGEXP$1.test(name);
}
const test$4$1 = (val) => val && val.constructor && !!val.constructor.name && testName$1(val.constructor.name);
function isNamedNodeMap$1(collection) {
	return collection.constructor.name === "NamedNodeMap";
}
const serialize$4$1 = (collection, config, indentation, depth, refs, printer) => {
	const name = collection.constructor.name;
	if (++depth > config.maxDepth) {
		return `[${name}]`;
	}
	return (config.min ? "" : name + SPACE$1$1) + (OBJECT_NAMES$1.has(name) ? `{${printObjectProperties$1(isNamedNodeMap$1(collection) ? [...collection].reduce((props, attribute) => {
		props[attribute.name] = attribute.value;
		return props;
	}, {}) : { ...collection }, config, indentation, depth, refs, printer)}}` : `[${printListItems$1([...collection], config, indentation, depth, refs, printer)}]`);
};
const plugin$4$1 = {
	serialize: serialize$4$1,
	test: test$4$1
};
...
function printProps$2(keys, props, config, indentation, depth, refs, printer) {
	const indentationNext = indentation + config.indent;
...
function printChildren$2(children, config, indentation, depth, refs, printer) {
	return children.map((child) => config.spacingOuter + indentation + (typeof child === "string" ? printText$2(child, config) : printer(child, config, indentation, depth, refs))).join("");
}
function printShadowRoot(children, config, indentation, depth, refs, printer) {
	if (config.printShadowRoot === false) {
		return "";
	}
	return [`${config.spacingOuter + indentation}#shadow-root`, printChildren$2(children, config, indentation + config.indent, depth, refs, printer)].join("");
}
function printText$2(text, config) {
	const contentColor = config.colors.content;
	return contentColor.open + escapeHTML$3(text) + contentColor.close;
}
function printComment$2(comment, config) {
...
function printElement$2(type, printedProps, printedChildren, config, indentation) {
	const tagColor = config.colors.tag;
	return `${tagColor.open}<${type}${printedProps && tagColor.close + printedProps + config.spacingOuter + indentation + tagColor.open}${printedChildren ? `>${tagColor.close}${printedChildren}${config.spacingOuter}${indentation}${tagColor.open}</${type}` : `${printedProps && !config.min ? "" : " "}/`}>${tagColor.close}`;
}
function printElementAsLeaf$2(type, config) {
	const tagColor = config.colors.tag;
	return `${tagColor.open}<${type}${tagColor.close} …${tagColor.open} />${tagColor.close}`;
}

const ELEMENT_NODE$3 = 1;
const TEXT_NODE$3 = 3;
const COMMENT_NODE$3 = 8;
const FRAGMENT_NODE$2 = 11;
const ELEMENT_REGEXP$2 = /^(?:(?:HTML|SVG)\w*)?Element$/;
...
function testNode$2(val) {
	const constructorName = val.constructor.name;
	const { nodeType, tagName } = val;
	const isCustomElement = typeof tagName === "string" && tagName.includes("-") || testHasAttribute$1(val);
	return nodeType === ELEMENT_NODE$3 && (ELEMENT_REGEXP$2.test(constructorName) || isCustomElement) || nodeType === TEXT_NODE$3 && constructorName === "Text" || nodeType === COMMENT_NODE$3 && constructorName === "Comment" || nodeType === FRAGMENT_NODE$2 && constructorName === "DocumentFragment";
}
const test$3$1 = (val) => val?.constructor?.name && testNode$2(val);
function nodeIsText$2(node) {
	return node.nodeType === TEXT_NODE$3;
}
function nodeIsComment$2(node) {
	return node.nodeType === COMMENT_NODE$3;
}
function nodeIsFragment$2(node) {
	return node.nodeType === FRAGMENT_NODE$2;
}
const serialize$3$1 = (node, config, indentation, depth, refs, printer) => {
	if (nodeIsText$2(node)) {
		return printText$2(node.data, config);
	}
	if (nodeIsComment$2(node)) {
		return printComment$2(node.data, config);
	}
	const type = nodeIsFragment$2(node) ? "DocumentFragment" : node.tagName.toLowerCase();
	if (++depth > config.maxDepth) {
		return printElementAsLeaf$2(type, config);
	}
	return printElement$2(type, printProps$2(nodeIsFragment$2(node) ? [] : Array.from(node.attributes, (attr) => attr.name).sort(), nodeIsFragment$2(node) ? {} : [...node.attributes].reduce((props, attribute) => {
		props[attribute.name] = attribute.value;
		return props;
	}, {}), config, indentation + config.indent, depth, refs, printer), (nodeIsFragment$2(node) || !node.shadowRoot ? "" : printShadowRoot(Array.prototype.slice.call(node.shadowRoot.children), config, indentation + config.indent, depth, refs, printer)) + printChildren$2(Array.prototype.slice.call(node.childNodes || node.children), config, indentation + config.indent, depth, refs, printer), config, indentation);
};
const plugin$3$1 = {
	serialize: serialize$3$1,
	test: test$3$1
};

// SENTINEL constants are from https://github.com/facebook/immutable-js
const IS_ITERABLE_SENTINEL$1 = "@@__IMMUTABLE_ITERABLE__@@";
const IS_LIST_SENTINEL$2 = "@@__IMMUTABLE_LIST__@@";
const IS_KEYED_SENTINEL$2 = "@@__IMMUTABLE_KEYED__@@";
const IS_MAP_SENTINEL$1 = "@@__IMMUTABLE_MAP__@@";
const IS_ORDERED_SENTINEL$2 = "@@__IMMUTABLE_ORDERED__@@";
const IS_RECORD_SENTINEL$1 = "@@__IMMUTABLE_RECORD__@@";
const IS_SEQ_SENTINEL$1 = "@@__IMMUTABLE_SEQ__@@";
const IS_SET_SENTINEL$2 = "@@__IMMUTABLE_SET__@@";
const IS_STACK_SENTINEL$1 = "@@__IMMUTABLE_STACK__@@";
const getImmutableName$1 = (name) => `Immutable.${name}`;
const printAsLeaf$1 = (name) => `[${name}]`;
const SPACE$3 = " ";
const LAZY$1 = "…";
function printImmutableEntries$1(val, config, indentation, depth, refs, printer, type) {
	return ++depth > config.maxDepth ? printAsLeaf$1(getImmutableName$1(type)) : `${getImmutableName$1(type) + SPACE$3}{${printIteratorEntries$1(val.entries(), config, indentation, depth, refs, printer)}}`;
}
...
function printImmutableRecord$1(val, config, indentation, depth, refs, printer) {
	// _name property is defined only for an Immutable Record instance
	// which was constructed with a second optional descriptive name arg
	const name = getImmutableName$1(val._name || "Record");
	return ++depth > config.maxDepth ? printAsLeaf$1(name) : `${name + SPACE$3}{${printIteratorEntries$1(getRecordEntries$1(val), config, indentation, depth, refs, printer)}}`;
}
function printImmutableSeq$1(val, config, indentation, depth, refs, printer) {
	const name = getImmutableName$1("Seq");
	if (++depth > config.maxDepth) {
		return printAsLeaf$1(name);
	}
	if (val[IS_KEYED_SENTINEL$2]) {
		return `${name + SPACE$3}{${val._iter || val._object ? printIteratorEntries$1(val.entries(), config, indentation, depth, refs, printer) : LAZY$1}}`;
	}
	return `${name + SPACE$3}[${val._iter || val._array || val._collection || val._iterable ? printIteratorValues$1(val.values(), config, indentation, depth, refs, printer) : LAZY$1}]`;
}
function printImmutableValues$1(val, config, indentation, depth, refs, printer, type) {
	return ++depth > config.maxDepth ? printAsLeaf$1(getImmutableName$1(type)) : `${getImmutableName$1(type) + SPACE$3}[${printIteratorValues$1(val.values(), config, indentation, depth, refs, printer)}]`;
}
const serialize$2$1 = (val, config, indentation, depth, refs, printer) => {
	if (val[IS_MAP_SENTINEL$1]) {
		return printImmutableEntries$1(val, config, indentation, depth, refs, printer, val[IS_ORDERED_SENTINEL$2] ? "OrderedMap" : "Map");
	}
	if (val[IS_LIST_SENTINEL$2]) {
		return printImmutableValues$1(val, config, indentation, depth, refs, printer, "List");
	}
	if (val[IS_SET_SENTINEL$2]) {
		return printImmutableValues$1(val, config, indentation, depth, refs, printer, val[IS_ORDERED_SENTINEL$2] ? "OrderedSet" : "Set");
	}
	if (val[IS_STACK_SENTINEL$1]) {
		return printImmutableValues$1(val, config, indentation, depth, refs, printer, "Stack");
	}
	if (val[IS_SEQ_SENTINEL$1]) {
		return printImmutableSeq$1(val, config, indentation, depth, refs, printer);
	}
	// For compatibility with immutable v3 and v4, let record be the default.
	return printImmutableRecord$1(val, config, indentation, depth, refs, printer);
};
// Explicitly comparing sentinel properties to true avoids false positive
// when mock identity-obj-proxy returns the key as the value for any key.
const test$2$1 = (val) => val && (val[IS_ITERABLE_SENTINEL$1] === true || val[IS_RECORD_SENTINEL$1] === true);
const plugin$2$1 = {
	serialize: serialize$2$1,
	test: test$2$1
};
...
var reactIsExports$2 = requireReactIs();
var index = /*@__PURE__*/getDefaultExportFromCjs$3(reactIsExports$2);
...
const ReactIs$1 = Object.fromEntries(reactIsMethods.map((m) => [m, (v) => ReactIs18[m](v) || ReactIs19[m](v)]));
// Given element.props.children, or subtree during recursive traversal,
// return flattened array of children.
function getChildren$1(arg, children = []) {
	if (Array.isArray(arg)) {
		for (const item of arg) {
			getChildren$1(item, children);
		}
...
function getType$3(element) {
	...
	if (ReactIs$1.isFragment(element)) {
		return "React.Fragment";
	}
	if (ReactIs$1.isSuspense(element)) {
		return "React.Suspense";
	}
	if (typeof type === "object" && type !== null) {
		if (ReactIs$1.isContextProvider(element)) {
			return "Context.Provider";
		}
		if (ReactIs$1.isContextConsumer(element)) {
			return "Context.Consumer";
		}
		if (ReactIs$1.isForwardRef(element)) {
			if (type.displayName) {
...
const serialize$1$2 = (element, config, indentation, depth, refs, printer) => ++depth > config.maxDepth ? printElementAsLeaf$2(getType$3(element), config) : printElement$2(getType$3(element), printProps$2(getPropKeys$1$1(element), element.props, config, indentation + config.indent, depth, refs, printer), printChildren$2(getChildren$1(element.props.children), config, indentation + config.indent, depth, refs, printer), config, indentation);
const test$1$1 = (val) => val != null && ReactIs$1.isElement(val);
const plugin$1$1 = {
	serialize: serialize$1$2,
	test: test$1$1
};

const testSymbol$1 = typeof Symbol === "function" && Symbol.for ? Symbol.for("react.test.json") : 245830487;
function getPropKeys$2(object) {
	const { props } = object;
	return props ? Object.keys(props).filter((key) => props[key] !== undefined).sort() : [];
}
const serialize$8 = (object, config, indentation, depth, refs, printer) => ++depth > config.maxDepth ? printElementAsLeaf$2(object.type, config) : printElement$2(object.type, object.props ? printProps$2(getPropKeys$2(object), object.props, config, indentation + config.indent, depth, refs, printer) : "", object.children ? printChildren$2(object.children, config, indentation + config.indent, depth, refs, printer) : "", config, indentation);
const test$a = (val) => val && val.$$typeof === testSymbol$1;
const plugin$8 = {
...
* when mock identity-obj-proxy returns the key as the value for any key.
*/
function getConstructorName$2(val) {
...
function printBasicValue$1(val, printFunctionName, escapeRegex, escapeString) {
	...
	if (toStringed === "[object Date]") {
		return Number.isNaN(+val) ? "Date { NaN }" : toISOString$1.call(val);
	}
	...
	if (toStringed === "[object RegExp]") {
		if (escapeRegex) {
			...
			return regExpToString$1.call(val).replaceAll(/[$()*+.?[\\\]^{|}]/g, "\\$&");
		}
...
* maps and sets etc )
*/
function printComplexValue$1(val, config, indentation, depth, refs, hasCalledToJSON) {
	...
	if (config.callToJSON && !hitMaxDepth && val.toJSON && typeof val.toJSON === "function" && !hasCalledToJSON) {
		return printer$1(val.toJSON(), config, indentation, depth, refs, true);
	}
	const toStringed = toString$3.call(val);
	if (toStringed === "[object Arguments]") {
		return hitMaxDepth ? "[Arguments]" : `${min ? "" : "Arguments "}[${printListItems$1(val, config, indentation, depth, refs, printer$1)}]`;
	}
	if (isToStringedArrayType$1(toStringed)) {
		return hitMaxDepth ? `[${val.constructor.name}]` : `${min ? "" : !config.printBasicPrototype && val.constructor.name === "Array" ? "" : `${val.constructor.name} `}[${printListItems$1(val, config, indentation, depth, refs, printer$1)}]`;
	}
	if (toStringed === "[object Map]") {
		return hitMaxDepth ? "[Map]" : `Map {${printIteratorEntries$1(val.entries(), config, indentation, depth, refs, printer$1, " => ")}}`;
	}
	if (toStringed === "[object Set]") {
		return hitMaxDepth ? "[Set]" : `Set {${printIteratorValues$1(val.values(), config, indentation, depth, refs, printer$1)}}`;
	}
	// Avoid failure to serialize global window object in jsdom test environment.
	// For example, not even relevant if window is prop of React element.
	return hitMaxDepth || isWindow$1(val) ? `[${getConstructorName$2(val)}]` : `${min ? "" : !config.printBasicPrototype && getConstructorName$2(val) === "Object" ? "" : `${getConstructorName$2(val)} `}{${printObjectProperties$1(val, config, indentation, depth, refs, printer$1)}}`;
}
const ErrorPlugin = {
	...
	serialize(val, config, indentation, depth, refs, printer) {
		...
		const name = val.name !== "Error" ? val.name : getConstructorName$2(val);
		return hitMaxDepth ? `[${name}]` : `${name} {${printIteratorEntries$1(Object.entries(entries).values(), config, indentation, depth, refs, printer)}}`;
	}
};
function isNewPlugin$1(plugin) {
	return plugin.serialize != null;
}
function printPlugin$1(plugin, val, config, indentation, depth, refs) {
	let printed;
	try {
		printed = isNewPlugin$1(plugin) ? plugin.serialize(val, config, indentation, depth, refs, printer$1) : plugin.print(val, (valChild) => printer$1(valChild, config, indentation, depth, refs), (str) => {
			const indentationNext = indentation + config.indent;
...
function findPlugin$1(plugins, val) {
...
function printer$1(val, config, indentation, depth, refs, hasCalledToJSON) {
	const plugin = findPlugin$1(config.plugins, val);
	if (plugin !== null) {
		return printPlugin$1(plugin, val, config, indentation, depth, refs);
	}
	const basicResult = printBasicValue$1(val, config.printFunctionName, config.escapeRegex, config.escapeString);
	if (basicResult !== null) {
		return basicResult;
	}
	return printComplexValue$1(val, config, indentation, depth, refs, hasCalledToJSON);
}
...
function validateOptions$1(options) {
	for (const key of Object.keys(options)) {
		if (!Object.hasOwn(DEFAULT_OPTIONS$1, key)) {
...
function getColorsHighlight$1() {
	return DEFAULT_THEME_KEYS$1.reduce((colors, key) => {
		const value = DEFAULT_THEME$1[key];
...
function getConfig$3(options) {
	return {
		callToJSON: options?.callToJSON ?? DEFAULT_OPTIONS$1.callToJSON,
		colors: options?.highlight ? getColorsHighlight$1() : getColorsEmpty$1(),
		...
		indent: options?.min ? "" : createIndent$1(options?.indent ?? DEFAULT_OPTIONS$1.indent),
...
function format$2(val, options) {
	if (options) {
		...
		if (options.plugins) {
			const plugin = findPlugin$1(options.plugins, val);
			if (plugin !== null) {
				return printPlugin$1(plugin, val, getConfig$3(options), "", 0, []);
			}
		}
	}
	const basicResult = printBasicValue$1(val, getPrintFunctionName$1(options), getEscapeRegex$1(options), getEscapeString$1(options));
	if (basicResult !== null) {
		return basicResult;
	}
	return printComplexValue$1(val, getConfig$3(options), "", 0, []);
}
const plugins$1 = {
	AsymmetricMatcher: plugin$5$1,
	DOMCollection: plugin$4$1,
	DOMElement: plugin$3$1,
	Immutable: plugin$2$1,
	ReactElement: plugin$1$1,
	ReactTestComponent: plugin$8,
	Error: ErrorPlugin
};
...
const { AsymmetricMatcher: AsymmetricMatcher$4, DOMCollection: DOMCollection$4, DOMElement: DOMElement$3, Immutable: Immutable$3, ReactElement: ReactElement$3, ReactTestComponent: ReactTestComponent$3 } = plugins$1;
const PLUGINS$2 = [
...
const { AsymmetricMatcher: AsymmetricMatcher$3, DOMCollection: DOMCollection$3, DOMElement: DOMElement$2, Immutable: Immutable$2, ReactElement: ReactElement$2, ReactTestComponent: ReactTestComponent$2 } = plugins$1;
const PLUGINS$1 = [
...
function isAsymmetricMatcher(data) {
	const type = getType$2(data);
	return type === "Object" && typeof data.asymmetricMatch === "function";
}
function isReplaceable(obj1, obj2) {
	const obj1Type = getType$2(obj1);
	const obj2Type = getType$2(obj2);
...
var _Assertion = class _Assertion {
  ...
  assert(_expr, msg, _negateMsg, expected, _actual, showDiff) {
    const ok = test$9(this, arguments);
...
function addCustomEqualityTesters(newTesters) {
	if (!Array.isArray(newTesters)) {
		throw new TypeError(`expect.customEqualityTesters: Must be set to an array of Testers. Was given "${getType$2(newTesters)}"`);
	}
...
function eq(a, b, aStack, bStack, customTesters, hasKey) {
	...
	const aKeys = keys$2(a, hasKey);
	let key;
	let size = aKeys.length;
	// Ensure that both objects contain the same number of properties before comparing deep equality.
	if (keys$2(b, hasKey).length !== size) {
		return false;
	}
	while (size--) {
		key = aKeys[size];
...
function isImmutableOrderedKeyed(source) {
	return Boolean(source && isObjectLiteral(source) && source[IS_KEYED_SENTINEL$1] && source[IS_ORDERED_SENTINEL$1]);
}
function isImmutableOrderedSet(source) {
	return Boolean(source && isObjectLiteral(source) && source[IS_SET_SENTINEL$1] && source[IS_ORDERED_SENTINEL$1]);
}
...
let AsymmetricMatcher$2 = class AsymmetricMatcher {
...
class StringContaining extends AsymmetricMatcher$2 {
	constructor(sample, inverse = false) {
...
class Anything extends AsymmetricMatcher$2 {
	asymmetricMatch(other) {
		return other != null;
	}
	toString() {
...
class ObjectContaining extends AsymmetricMatcher$2 {
	constructor(sample, inverse = false) {
		super(sample, inverse);
	}
	getPrototype(obj) {
...
class ArrayContaining extends AsymmetricMatcher$2 {
	constructor(sample, inverse = false) {
		super(sample, inverse);
	}
	asymmetricMatch(other, customTesters) {
...
class Any extends AsymmetricMatcher$2 {
	constructor(sample) {
...
class StringMatching extends AsymmetricMatcher$2 {
	constructor(sample, inverse = false) {
...
class CloseTo extends AsymmetricMatcher$2 {
	precision;
...
class SchemaMatching extends AsymmetricMatcher$2 {
	result;
	constructor(sample, inverse = false) {
...
function formatCalls(spy, msg, showActualCall) {
	if (spy.mock.calls.length) {
		msg += y.gray(`\n\nReceived: \n\n${spy.mock.calls.map((callArg, i) => {
			let methodCall = y.bold(`  ${ordinalOf(i + 1)} ${spy.getMockName()} call:\n\n`);
			if (showActualCall) {
...
function formatReturns(spy, results, msg, showActualReturn) {
	if (results.length) {
		msg += y.gray(`\n\nReceived: \n\n${results.map((callReturn, i) => {
			let methodCall = y.bold(`  ${ordinalOf(i + 1)} ${spy.getMockName()} call return:\n\n`);
			if (showActualReturn) {
...
* ```
*/
function beforeAll$1(fn, timeout = getDefaultHookTimeout()) {
...
* ```
*/
function afterAll$1(fn, timeout) {
...
* ```
*/
function afterEach$1(fn, timeout) {
...
* ```
*/
const test$8 = createTest(function(name, optionsOrFn, optionsOrTest) {
...
function createTaskCollector(fn, context) {
	...
	taskFn.afterEach = afterEach$1;
	taskFn.beforeAll = beforeAll$1;
	taskFn.afterAll = afterAll$1;
...
function formatTitle(template, items, idx) {
	...
	handleRegexMatch(
		...
		(match) => {
			if (i < count) {
				output += format$1(match[0], items[i++]);
			} else {
				output += match[0];
			}
		},
...
async function saveInlineSnapshots(environment, snapshots) {
	const MagicString = (await import('./magic-string.es.esm.js')).default;
...
const serialize$1$1 = (val, config, indentation, depth, refs, printer) => {
...
const test$7 = (val) => val && !!val._isMockFunction;
const plugin$7 = {
	serialize: serialize$1$1,
	test: test$7
};

const { DOMCollection: DOMCollection$2, DOMElement: DOMElement$1, Immutable: Immutable$1, ReactElement: ReactElement$1, ReactTestComponent: ReactTestComponent$1, AsymmetricMatcher: AsymmetricMatcher$1 } = plugins$1;
let PLUGINS = [
...
function serialize$7(val, indent = 2, formatOverrides = {}) {
	return normalizeNewlines(format$2(val, {
		escapeRegex,
		indent,
...
class SnapshotState {
	...
	match({ testId, testName, received, key, inlineSnapshot, isInline, error, rawSnapshot }) {
		...
		let receivedSerialized = rawSnapshot && typeof received === "string" ? received : serialize$7(received, undefined, this._snapshotFormat);
		if (!rawSnapshot) {
...
function requireGlobal () {
	if (hasRequiredGlobal) return global$3;
	hasRequiredGlobal = 1;
...
export { afterEach$1 as a, cleanup as c, describe as d, globalExpect as g, it as i, render as r };
